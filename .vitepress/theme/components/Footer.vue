<template>
  <footer
    class="bg-alt f12 lh-normal"
    style="display: flex; flex-direction: column; align-items: center"
  >
    <div
      class="flex flex-grow-1 ma2"
      style="display: flex; justify-content: center; width: 100%"
    >
      <div class="ma2" style="text-align: center">
        <a title="Home" target="_blank" href="/">
          <img
            role="img"
            width="48"
            height="48"
            src="https://m.theovan.cn/avatar.png"
          />
        </a>
      </div>
    </div>

    <div
      class="flex flex-grow-1 ma4"
      style="display: flex; justify-content: center; width: 100%"
    >
      <!-- GitHub Source -->
      <div class="fb6">
        <div class="mb2 fw6 f14">GitHub Source</div>
        <ul>
          <li class="mb2">
            <a
              class="external-link"
              target="_blank"
              href="https://github.com/Theo-messi/Theo-Docs"
              >Theo-Docs</a
            >
          </li>
          <li class="mb2">
            <a
              class="external-link"
              target="_blank"
              href="https://github.com/Theo-messi/xx.theovan.cn"
              >玄学宝典</a
            >
          </li>
          <li class="mb2">
            <a
              class="external-link"
              target="_blank"
              href="https://github.com/Theo-messi/share.theovan.cn"
              >阿里云盘资源分享</a
            >
          </li>
          <li class="mb2">
            <a
              class="external-link"
              target="_blank"
              href="https://github.com/Theo-messi/own-rules"
              >自用 Clash 配置</a
            >
          </li>
        </ul>
      </div>
      <!-- 合作伙伴 -->
      <div class="fb6">
        <div class="mb2 fw6 f14">合作伙伴</div>
        <ul>
          <li class="mb2">
            <a
              class="external-link"
              target="_blank"
              href="https://nf.video/kaIuE"
              >银河录像局</a
            >
          </li>
          <li class="mb2">
            <a
              class="external-link"
              target="_blank"
              href="https://ihezu.love/UKTer6"
              >奈飞小铺</a
            >
          </li>
          <li class="mb2">
            <a
              class="external-link"
              target="_blank"
              href="https://fbinv01.fbaff.cc/auth/register?code=RZP3"
              >FlyingBird</a
            >
          </li>
          <li class="mb2">
            <a
              class="external-link"
              target="_blank"
              href="https://patriot.ninja/aff.php?aff=1471"
              >Bridge The Wise</a
            >
          </li>
          <li class="mb2">
            <a
              class="external-link"
              target="_blank"
              href="https://sms-activate.org/?ref=8170513"
              >SMS-Activate</a
            >
          </li>
        </ul>
      </div>
      <!-- 观影指南 -->
      <div class="fb6">
        <div class="mb2 fw6 f14">观影指南</div>
        <ul>
          <li class="mb2">
            <a target="_blank" href="/streaming/Netflix-guide">Netflix</a>
          </li>
          <li class="mb2">
            <a target="_blank" href="/streaming/Disney-introduce">Disney+</a>
          </li>
          <li class="mb2">
            <a target="_blank" href="/streaming/Spotify">Spotify Premium</a>
          </li>
          <li class="mb2">
            <a target="_blank" href="/streaming/YouTube">YouTube Premium</a>
          </li>
          <li class="mb2">
            <a target="_blank" href="/streaming/Hulu">Hulu</a>
          </li>
          <li class="mb2">
            <a target="_blank" href="/streaming/HBO-Max">HBO Max</a>
          </li>
        </ul>
      </div>
      <!-- 友情链接 -->
      <div class="fb6">
        <div class="mb2 fw6 f14">相关链接</div>
        <ul>
          <li class="mb2">
            <a class="external-link" target="_blank" href="https://theovan.cn/"
              >Theo-Docs</a
            >
          </li>
          <li class="mb2">
            <a
              class="external-link"
              target="_blank"
              href="https://xx.theovan.cn/"
              >玄学宝典</a
            >
          </li>
          <li class="mb2">
            <a
              class="external-link"
              target="_blank"
              href="https://share.theovan.cn/"
              >阿里云盘资源分享</a
            >
          </li>
          <li class="mb2">
            <a
              class="external-link"
              target="_blank"
              href="https://vitepress.dev/"
              >VitePress</a
            >
          </li>
        </ul>
      </div>
    </div>

    <!-- 底部信息栏 -->

    <div
      class="flex ma4"
      style="display: flex; align-items: center; text-align: center"
    >
      <span
        class="ml1"
        style="display: flex; align-items: center; text-decoration: none"
      >
        Copyright © 2019 - {{ new Date().getFullYear() }}
        <a class="ml0" target="_blank" href="https://github.com/Theo-messi"
          >Theo-Messi</a
        >. All Rights Reserved
      </span>
      <span
        class="ml1"
        style="display: flex; align-items: center; text-decoration: none"
      >
        <svg
          t="1718360166271"
          class="icon"
          viewBox="0 0 1024 1024"
          version="1.1"
          xmlns="http://www.w3.org/2000/svg"
          p-id="6209"
          width="16"
          height="16"
          style="margin-right: 4px"
        >
          <path
            d="M923.05 338.33a447.29 447.29 0 0 0-217.71-228.5q-9.71-4.66-19.67-8.88a447.5 447.5 0 0 0-347.32 0q-20.28 8.58-39.5 19a442.63 442.63 0 0 0-79.41 55.21q-11.77 10.23-22.89 21.36a445.77 445.77 0 0 0 0 630.92q11.21 11.2 23.08 21.53a443 443 0 0 0 99.47 65.38q9.5 4.56 19.24 8.68a447.38 447.38 0 0 0 340 3c2.42-1 4.85-2 7.26-3a446.56 446.56 0 0 0 237.45-584.7zM662.29 156.2a384.42 384.42 0 0 1 68.13 37.3c-6.26 21.25-21.83 41.52-45.53 59.08q-7.41-21.11-16.08-40.38c-11.88-26.38-25.18-49.2-39.64-68.2q16.77 5.29 33.12 12.2z m2.28 325.8H542V362.52c37.29-3 73-10.74 105.29-22.73 9.71 44.02 15.6 92.11 17.28 142.21z m-305.15 60H482v108.11c-38.13 2.1-74.79 9.11-108.14 20.55A799.3 799.3 0 0 1 359.42 542z m16.5-198.58c32.79 11 68.73 17.8 106.08 19.85V482H359.42c1.64-48.72 7.26-95.55 16.5-138.58zM542 542h122.57a795.74 795.74 0 0 1-15.19 132.37c-32.87-12.43-69.28-20.44-107.38-23.51z m0-398.2c25.91 14.3 51.14 46.5 72.1 93a460.89 460.89 0 0 1 17.33 44.82c-27 10.65-57.37 17.67-89.43 20.65z m-60 0v159.38c-32.26-2-63-8.12-90.64-17.85a468.59 468.59 0 0 1 18.53-48.49c20.96-46.54 46.19-78.74 72.11-93.04z m-120.29 12.4q16.29-6.91 33.12-12.2c-14.46 19.05-27.77 41.87-39.64 68.25-6.63 14.72-12.71 30.3-18.23 46.62-26.18-17.24-44-37.52-52.3-59a383.92 383.92 0 0 1 77.05-43.67zM156.2 361.71a384.64 384.64 0 0 1 81.38-121.35q3.27 5.85 7.12 11.58c15.3 22.75 36.84 43 64 60.05q5.53 3.48 11.3 6.74c-11.76 50.5-18.8 105.82-20.62 163.27H127a382.41 382.41 0 0 1 29.2-120.29z m0 300.58A382.54 382.54 0 0 1 127 542h172.4c1.71 53.91 8 105.94 18.5 153.87-3.12 1.79-6.17 3.63-9.18 5.52-27.18 17.1-48.72 37.3-64 60.05A160.93 160.93 0 0 0 234 779.92a384.53 384.53 0 0 1-77.8-117.63z m205.51 205.5a384.1 384.1 0 0 1-79.82-45.65c6.25-23.84 24.21-46.54 52.21-65.65a526.35 526.35 0 0 0 21.09 55.3c11.87 26.38 25.18 49.2 39.64 68.25q-16.83-5.34-33.12-12.25zM482 880.19c-25.92-14.3-51.15-46.49-72.11-93a478 478 0 0 1-21.51-58c28.42-10.34 60.2-16.79 93.62-18.91z m60 0V711.07c33.26 3.09 64.66 10.53 92.45 21.85a473.57 473.57 0 0 1-20.35 54.24c-20.96 46.54-46.19 78.73-72.1 93.03z m120.29-12.4Q646 874.71 629.17 880c14.46-19 27.76-41.87 39.64-68.25a513.8 513.8 0 0 0 19-48.82c25.36 19.59 40.81 42.38 44.76 66.06a383.56 383.56 0 0 1-70.28 38.8z m205.5-205.5A384.82 384.82 0 0 1 785 785q-1.28 1.28-2.55 2.52a157.18 157.18 0 0 0-14.26-26.11c-15.19-22.59-36.52-42.66-63.43-59.68 11.27-49.55 18-103.64 19.81-159.76H897a382.26 382.26 0 0 1-29.21 120.32zM724.6 482c-1.89-59.64-9.41-117-22-169l1.59-1c27.18-17.09 48.71-37.3 64-60.05a159.1 159.1 0 0 0 10.89-18.77c2 1.91 4 3.84 5.92 5.79A384.76 384.76 0 0 1 897 482z"
            fill="#1B7AFF"
            p-id="6210"
          ></path>
        </svg>
        ICP粤
        <a target="_blank" href="https://theovan.cn/" style="margin-left: 4px"
          >B2-20090059号</a
        >
      </span>
      <span
        class="ml1"
        style="display: flex; align-items: center; text-decoration: none"
      >
        <img
          role="img"
          width="16"
          height="16"
          src="https://inews.gtimg.com/newsapp_bt/0/0613150938895_706/0"
          alt="avatar"
          style="margin-right: 4px"
        />
        粤公网安备
        <a target="_blank" href="https://theovan.cn/" style="margin-left: 4px"
          >44030002000001号</a
        >
      </span>
    </div>
  </footer>
</template>
