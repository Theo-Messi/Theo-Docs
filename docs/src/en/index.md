---
layout: home
title: Home
titleTemplate: Theo Notes
hero:
  name: Note library
  text:
  tagline: You don't have to be the same
  image: /home.png
  actions:
    - theme: brand
      text: Front-end Nav
      link: nav

    - theme: alt
      text: Front-end Story
      link: /en/fe/javascript/types

    - theme: brand
      text: Documentation
      link: /en/skill/JL-01

    - theme: sponsor
      text: sponsor
      link: https://github.com/sponsors/Theo-messi
features:
  - title: Documentation
    icon:
      src: https://vitejs.dev/logo-with-shadow.png
      width: 30
    details: daily records
    link: /en/skill/JL-01
    linkText: Documentation

  - title: Front-end story
    icon:
      src: /fe.png
      width: 30
    details: Organize common front-end knowledge points <small>(Interview eight-part essay)</small><br/>"If you have any objections, it will be based on your understanding."
    link: /en/fe/javascript/types
    linkText: Common front-end knowledge

  - title: VPS User Guide
    icon:
      src: /VPS.png
      width: 40
    details: Basic settings/advanced usage/building services
    link: /en/VPS/01
    linkText: VPS User Guide

  - title: ESXI Installation Guide
    icon:
      src: data:image/svg+xml;base64,
      width: 30
    details: Full analysis of ESXI installation process</br>Installing OpenWrt...
    link: /en/ESXI/ESXI-01
    linkText: ESXI Installation Guide

  - title: Qinglong panel
    icon:
      src: https://qn.whyour.cn/logo.png
      width: 30
    details: Install Qinglong Panel</br>Build JD.com Sign-in and Alibaba Cloud Disk Sign-in</br> Obtain <small>JD.com Cookie/Alibaba Cloud Disk Cookie</small>
    link: /en/Docker/QL-01
    linkText: Qinglong panel

  - title: Metaphysical Book
    icon:
      src: /bagua.png
      width: 30
    details: Classic Chinese books on traditional Chinese culture, including traditional five arts (divination by mountain doctors) and classic literary works of various schools.
    link: https://xx.theovan.xyz
    linkText: Metaphysical Book
---
